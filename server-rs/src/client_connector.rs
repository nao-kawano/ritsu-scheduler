...
impl ClientConnector {
    ...
    fn thread_receiver(
        ...
        println!("ClientConnector: receive thread started.");
        let mut recv_buf = [0u8; MESSAGE_LEN_MAX];
        loop {
            // check stop request.
            if stop_flag.load(Ordering::Relaxed) == true {
                println!("ClientConnector: stop request detected, exitting");
                break;
            }
            ...
            match socket.recv_from(&mut recv_buf) {
                ...
                Err(e) => {
                    if e.kind() == std::io::ErrorKind::TimedOut {
                        ...
                        println!("Connector: failed to read: {:?}", e);
                        // todo: report and shutdown.
                    }
                }
            }
        }
        println!("ClientConnector: receive thread stopped.");
    }
    ...
    pub fn start(&mut self, tx_channel: Sender<Event>) -> bool {
        ...
        let s = match ClientConnector::create_socket(self.port) {
            Ok(socket) => socket,
            Err(e) => {
                println!("ClientConnector: Failed to create socket: {:?}", e);
                return false;
            }
        };
    ...
    pub fn stop(&mut self) {
        if let Some(handle) = self.thread_handle.take() {
            println!("ClientConnector: stopping...");
            self.stop_flag.store(true, Ordering::Relaxed);
            handle.join().unwrap();
            println!("ClientConnector: stopped");
            self.stop_flag.store(false, Ordering::Relaxed);
        } else {
            println!("ClientConnector: not started");
        }
    }
        ...
        if let Some(socket) = &self.socket {
            ...
            for msg in msgs {
                if let Some(to_addr) = clients.get(&msg.client_id) {
                    ...
                    println!("Connector: client is not connected id={}", msg.client_id);
                }
            }
        } else {
            println!("Connector: not started");
        }
    }
}
