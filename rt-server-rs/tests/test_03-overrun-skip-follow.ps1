...
$ScenarioName   = "03-overrun-skip-follow"
$ConfigTemplate = "test_03-overrun-skip-follow.toml"
$WaitTimeMs     = 800
$ServerPort     = 7903
...
$Clients = @(
    @{ Id = 10; ProcTime = "0.01"; ProcCount = 5 },   # Lead (will be skipped due to follower overrun, trigger exit)
    @{ Id = 11; ProcTime = "0.11"; ProcCount = 10 },  # Follower: overrun client (will cause cascaded skip on lead in next cycle)
...
if (Test-Path $ServerLogPath) {
    ...
    foreach ($line in $content) {
        ...
        if ($line -match 'CID:011.*Running -> Overrun') {
            $hasOverrun = $true
        }
        if ($line -match 'CID:010.*Ready -> Skip') {
            $hasSkip = $true
        }
        if ($line -match 'CID:011.*Overrun -> Late') {
            $hasLate = $true
        }
    }
    if ($hasError) {
